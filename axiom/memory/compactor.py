...
class MemoryCompactor:
    ...
    async def run_compaction(self) -> Dict[str, Any]:
        ...
        """
        # Fetch all memory embeddings
        cursor = await self._db.execute(
            "SELECT e.id as emb_id, e.owner_id as memory_key, e.embedding_json, "
            "m.created_at, m.retrieval_count, m.confidence_weight, m.value_json "
            "FROM embeddings e "
            "JOIN memories m ON e.owner_id = m.key AND e.owner_type = 'memory'"
        )
        ...
        for i in range(len(vectors)):
            ...
            for j in range(i + 1, len(vectors)):
                ...
                if sim > self.similarity_threshold:
                    ...
                    # Resolution: newer gets max retrieval_count + confidence boost
                    ...
                    await self._db.execute(
                        "UPDATE memories SET retrieval_count = ?, confidence_weight = ? WHERE key = ?",
                        (new_retrieval_count, new_confidence, newer["key"])
                    )
                    ...
                    # Update the newer metadata dict so future iterations within the same scan use updated counts
