...
class QdrantLocalStore:
    ...
    # Expected vector dimension (Ollama neural-chat typically uses 4096, but it varies by model. 
    # Qdrant requires defining the vector size upon collection creation if it doesn't exist.
    # In a flexible system, we might need to handle varying dimensions, but for simplicity
    # or dynamic sizes, we can use a large enough size or recreate if mismatched.
    # Actually, AXIOM's Ollama default `neural-chat` has a dimension of 4096. 
    # wait, Ollama embeddings typically use 4096 (Llama 2) or 3072 (text-embedding-3) or 768 (nomic-embed-text).
    # Since Qdrant requires a dimension at collection creation, we might need to lazy-init the collection
    # based on the first vector dimension, or set it to the model's known dimension.
    # Let's lazy-init the collection upon first upsert.
        ...
        try:
            ...
            for hit in results:
                ...
                formatted_results.append({
                    "id": payload.get("owner_id", str(hit.id)),
                    "owner_id": payload.get("owner_id", str(hit.id)),
