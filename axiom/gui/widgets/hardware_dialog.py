...
class HardwareMatrixDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        # Power Governor Group
        power_group = QGroupBox("Cognitive Power Governor")
        power_group.setStyleSheet("QGroupBox { font-weight: bold; color: #cdd6f4; border: 1px solid #45475a; margin-top: 10px; }")
        power_layout = QVBoxLayout()
        
        self.power_label = QLabel("🔋 Status: MAX PERFORMANCE (AC Power)")
        self.power_label.setStyleSheet("color: #a6e3a1; font-size: 14px; font-weight: bold;")
        power_layout.addWidget(self.power_label)
        
        self.model_label = QLabel("🧠 Active Base Model: llama3:8b (8.0B Parameters)")
        self.model_label.setStyleSheet("color: #cdd6f4;")
        ...
        # USB Interceptor Group
        usb_group = QGroupBox("USB/BLE Zero-Trust Interceptor")
        usb_group.setStyleSheet("QGroupBox { font-weight: bold; color: #cdd6f4; border: 1px solid #45475a; margin-top: 10px; }")
        ...
        self.usb_table.setStyleSheet("QTableWidget { background-color: #1e1e2e; color: #a6adc8; gridline-color: #313244; }")
    ...
    def _on_usb_intercept(self, event):
        data = event.data
        self._add_usb_row(data.get("node"), "USB Mass Storage", "SANDBOXED (Scanning)", "#f38ba8")
    ...
    def _on_power_state(self, event):
        ...
        if event.name == "power.state.critical":
            ...
            self.power_label.setStyleSheet("color: #f38ba8; font-size: 14px; font-weight: bold;")
    ...
    def _add_usb_row(self, node, type_str, status, color):
    ...
    def _simulate_usb(self):
        ...
        self._add_usb_row(node, "USB Flash Drive", "SANDBOXED (Scanning)", "#f38ba8")
        # Simulate release
