...
class ConsensusEngine:
    ...
    def __init__(self, event_bus: EventBus, blackboard=None, memory_store=None, session_id: Optional[str] = None):
        ...
        # Blackboard commit hook — if set, an APPROVED proposal will extract
        # the final agent artifact and persist it to memory_store.
        self._blackboard = blackboard
        self._memory_store = memory_store
        self._session_id = session_id
        
        # Subscribe to incoming votes
    ...
    def _handle_vote(self, event: Event) -> None:
        ...
        vote = event.data.get("vote")  # "APPROVED" or "REJECTED"
        ...
        if vote == "APPROVED":
            ...
            # Commit hook: extract final artifact from Blackboard → persistent memory
    ...
    def propose(self, agent_name: str, tool_name: str, arguments: Dict[str, Any], timeout: float = 5.0) -> bool:
        ...
        # Wait for resolution (simple polling for demonstration, bounded by timeout)
        ...
        # Deadlock mitigation: Timeout implies rejection
    ...
    def _commit_blackboard_artifact(self, proposal: Dict[str, Any]) -> None:
        ...
        # Collect all artifacts this agent wrote to the Blackboard this session
        ...
        for key in artifact_keys:
            ...
            # Format as a canonical memory message and persist
            content = (
                f"[Swarm Consensus Commit] Agent={agent_name}, Tool={tool_name}, "
                f"Artifact={key}: {str(value)[:500]}"
            )
            try:
                self._memory_store.add_message("system", content)
                logger.info(
                    "Consensus: Committed Blackboard artifact '%s' from agent '%s' to persistent memory.",
                    key, agent_name
                )
            except Exception as exc:
                logger.warning(
                    "Consensus: Failed to commit artifact '%s' to memory: %s",
                    key, exc
                )
