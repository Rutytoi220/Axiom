...
class FlightRecorder:
    ...
    def start(self) -> None:
        ...
        # Subscribe to all events via bus.published meta-event
    ...
    def stop(self) -> None:
        ...
        self._running = False
        self._queue.put(None)  # Sentinel value to wake up thread
        if self._thread:
    ...
    def _on_event(self, event: Event) -> None:
        ...
        if not self._running:
            return
        # Extract the original event data from the meta-event
        ...
        trace_record = {
            ...
            "event_type": orig_event,
            "data": self._scrub_secrets(orig_payload)
        }
        try:
            self._queue.put_nowait(trace_record)
        except queue.Full:
            pass  # Drop if we somehow get full (though Queue is unbounded by default)
    ...
    def _scrub_secrets(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            ...
            # Scrub Bearer tokens
            s = re.sub(r"Bearer [a-zA-Z0-9\-\._~+/]+", "Bearer ***", data)
            # Scrub API keys starting with sk-
            s = re.sub(r"sk-[a-zA-Z0-9]{20,}", "sk-***", s)
            # Scrub passwords in URLs
            s = re.sub(r"(://[^:]+:)([^@]+)(@)", r"\1***\3", s)
            # Mask absolute home paths
            try:
