...
class RoutineEngine:
    ...
    async def parse_schedule_to_cron(self, prompt: str) -> str:
        ...
        try:
            response = await asyncio.to_thread(
                self.cli.ollama.generate,
                prompt,
                system_prompt=sys_prompt
            )
            # The generate API might return a string directly or an object. Let's assume a string for now.
            cron_expr = response.strip()
            # Basic cleanup in case LLM added backticks
    ...
    async def _execute_routine(self, routine: Dict[str, Any]):
        ...
        try:
            # Dispatch to orchestrator silently
            def _run():
